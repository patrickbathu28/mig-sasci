...
class SasciController ( val sasciClient: SasciClient , val util: Utils ) {
    ...
    @PostMapping("/v1/cidadao", produces= arrayOf(MediaType.APPLICATION_JSON_VALUE))
    fun create(@RequestBody cidadaoDTO: CidadaoDTO) = ResponseEntity.status(HttpStatus.CREATED).body(sasciClient?.create(cidadaoDTO))
